...
module box2d.dynamics.b2Fixture;
...
struct b2FixtureProxy
{
	...
	b2Fixture fixture;
	int32 childIndex;
	int32 proxyId = b2BroadPhase.e_nullProxy;
};
...
public:
	...
	bool RayCast(ref b2RayCastOutput output, b2RayCastInput input, int32 childIndex) const
	{
	...
	/// may be expensive.
	void GetMassData(ref b2MassData massData) const
	{
			...
			case b2Shape.Type.e_polygon:
				{
					...
					b2Log("    auto shape = new b2PolygonShape;\n");
			...
			case b2Shape.Type.e_chain:
				{
					...
					b2Log("    auto shape = new b2ChainShape;\n");
					b2Log("    b2Vec2[%d] vs;\n", s.m_vertices.length);
					foreach(i, v; s.m_vertices)
					{
						b2Log("    vs[%d].Set(%.15lef, %.15lef);\n", i, v.x, v.y);
					}
					b2Log("    shape.CreateChain(vs);\n");
...
protected:
	...
	package void Create(b2Body pbody, const b2FixtureDef def)
	{
	...
	package void CreateProxies(b2BroadPhase broadPhase, b2Transform xf)
	{
		assert(m_proxyCount == 0);
	...
	package void DestroyProxies(b2BroadPhase broadPhase)
	{
	...
	package void Synchronize(b2BroadPhase broadPhase, b2Transform xf1, b2Transform xf2)
	{
		if (m_proxyCount == 0)
		{	
			return;
		...
		for (int32 i = 0; i < m_proxyCount; ++i)
		{
			...
			b2Vec2 displacement = xf2.p - xf1.p;

			broadPhase.MoveProxy(proxy.proxyId, proxy.aabb, displacement);
		}
	}

	package float32 m_density = 0.0f;

	package b2Fixture m_next;
	package b2Body m_body;
	...
	package b2FixtureProxy[] m_proxies;
	package int32 m_proxyCount = 0;
